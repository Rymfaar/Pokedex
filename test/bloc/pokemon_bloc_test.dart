...
void main() {
  ...
  group('Pokemon Bloc tests', () {
    blocTest<PokemonBloc, PokemonState>(
      ...
      setUp: () {
        when(pokemonRepository.fetchPokemons()).thenAnswer((_) => Future.value(TestPokemon.pokemons));
      },
      build: () => pokemonBloc,
    ...
    blocTest<PokemonBloc, PokemonState>(
      ...
      setUp: () {
        when(pokemonRepository.fetchPokemons()).thenAnswer((_) => Future.value([]));
      },
      build: () => pokemonBloc,
      act: (bloc) => bloc..add(PokemonFetched()),
      expect: () {
    ...
    blocTest<PokemonBloc, PokemonState>(
      ...
      setUp: () {
        when(pokemonRepository.fetchPokemons()).thenThrow(StateError('fail to fetch'));
      },
      build: () => pokemonBloc,
      act: (bloc) => bloc..add(PokemonFetched()),
      expect: () {
